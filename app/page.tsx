          ...
          {data.results.map((movie) => (
            <div className="flex flex-col overflow-hidden rounded-lg border bg-white">
              <Link className='group relative block h-48 overflow-hidden bg-gray-100 md:h-64' href={`/movie/${movie.id}`}>
                ...
                width={500}
                height={500}
                alt='image movie banner' className='absolute inset-0 h-full w-full object-cover object-center transition duration-200
                group-hover:scale-110' />
              </Link>
              <div className="flex flex-1 flex-col p-4 sm:p-6">
                <h2 className="mb-2 text-lg font-semibold text-gray-800">
                  <Link className='transition duration-100 hover:text-teal-500 active:text-teal-600' href={`/movie/${movie.id}`}>{movie.title}</Link>
                </h2>
